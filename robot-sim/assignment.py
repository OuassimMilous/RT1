...
def main():
    ...
    while 1:
        #if our robot isnt holding anything:
        if (grabbing == False):

            #we look for the closest free token
            code = scan(False)

            #if we find anything we go and get it
            if (code != -1 ):
                grabbing = go_get_it(code,True)
            else:

                #if we dont find any free tokens, we check if all of them are in the goal, if they are the game ends.
                if(len(goal) == len(unique_markers)):
                    break
                # if they arent, it means we just cant see the tokens so we turn to look for them
                if(code == -1):
                    turn(10,0.5)
    
         #if our robot isn holding something:               
        else:
            #we look for the closest goal
            code = scan(True)

            #if we find anything we go and get it
            if (code != -1):
                grabbing = go_get_it(code,False)
                #if we dont find any goal tokens, it means we just cant see the tokens so we turn to look for them
            else:
                if(code == -1):
                    turn(10,0.5)
